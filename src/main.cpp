...
bool FetchIgdbMetadataForGame(AppState* state, GameEntry& game, bool allow_cover_download);
...
std::wstring JoinWithSeparator(const std::vector<std::wstring>& items, const std::wstring& separator) {
  std::wstring result;
...
bool FetchIgdbMetadataForGame(AppState* state, GameEntry& game, bool allow_cover_download) {
  ...
  if (game.igdbMetadataFetched) {
    return false;
  }
  game.igdbMetadataFetched = true;

  if (state->settings.igdbClientId.empty() || state->settings.igdbClientSecret.empty()) {
    state->igdb_status =
        L"IGDB integration disabled. Provide a Client ID and Secret in Settings to fetch cover art and details.";
    UpdateStatusBar(state, state->igdb_status);
    return false;
  }

  std::wstring token = state->settings.igdbAccessToken;
  std::wstring expires = state->settings.igdbTokenExpiresUtc;
  std::wstring error;
  if (!IGDB::EnsureAccessToken(state->settings.igdbClientId, state->settings.igdbClientSecret, token, expires, error)) {
    if (!error.empty()) {
      state->igdb_status = error;
      UpdateStatusBar(state, state->igdb_status);
    } else {
      state->igdb_status = L"IGDB token request failed.";
      UpdateStatusBar(state, state->igdb_status);
    }
    return false;
  }

  if (token != state->settings.igdbAccessToken || expires != state->settings.igdbTokenExpiresUtc) {
    state->settings.igdbAccessToken = token;
    state->settings.igdbTokenExpiresUtc = expires;
  }

  std::wstring query_name = !game.name.empty() ? game.name : game.folder;
  auto igdb_game = IGDB::SearchOne(state->settings.igdbClientId, token, query_name);
  if (!igdb_game) {
    state->igdb_status = L"IGDB lookup failed for " + query_name;
    UpdateStatusBar(state, state->igdb_status);
    return false;
  }

  if (!igdb_game->name.empty()) {
    game.igdbTitle = igdb_game->name;
  }
  game.igdbSummary = igdb_game->summary;
  game.igdbGenres = igdb_game->genres;
  game.igdbPlatforms = igdb_game->platforms;
  if (igdb_game->totalRating.has_value()) {
    game.igdbHasRating = true;
    game.igdbRating = igdb_game->totalRating.value();
  }

  if (allow_cover_download && !igdb_game->coverUrl.empty()) {
    std::wstring cached = IGDB::CacheImage(igdb_game->coverUrl);
    if (!cached.empty()) {
      HBITMAP cover = CoverCache::LoadImage(cached, kTileWidth, kTileHeight);
      if (cover) {
        if (game.coverBmp) {
          DeleteObject(game.coverBmp);
        }
        game.coverBmp = cover;
        CoverCache::SaveForExe(game.coverBmp, game.exe);
      }
    }
  }

  if (!game.igdbTitle.empty()) {
    state->igdb_status = L"IGDB: " + game.igdbTitle + L" metadata loaded.";
  } else {
    state->igdb_status = L"IGDB metadata loaded.";
  }
  UpdateStatusBar(state, state->igdb_status);
  return true;
}
...
void ApplyConfigToGames(AppState* state) {
  ...
  for (auto& game : state->games) {
    ...
    FetchIgdbMetadataForGame(state, game, game.coverBmp == nullptr);
    if (!game.coverBmp) {
...
void UpdateIgdbStatusLabel(HWND dlg, const SettingsData& settings) {
  std::wstring status;
  if (settings.igdbClientId.empty() || settings.igdbClientSecret.empty()) {
    status = L"Not configured.";
  } else if (settings.igdbAccessToken.empty()) {
    status = L"Token not fetched.";
  } else {
    status = L"Token expires " + (settings.igdbTokenExpiresUtc.empty() ? L"(unknown)" : settings.igdbTokenExpiresUtc);
  }
...
void ApplySettingsFromDialog(HWND dlg, SettingsDialogContext* context) {
  ...
  std::wstring igdb_secret = Trim(GetDlgItemTextString(dlg, IDC_SETTINGS_IGDB_CLIENT_SECRET));
  bool credentials_changed = (igdb_id != settings.igdbClientId || igdb_secret != settings.igdbClientSecret);
  settings.igdbClientId = igdb_id;
  settings.igdbClientSecret = igdb_secret;
  SetDlgItemTextString(dlg, IDC_SETTINGS_IGDB_CLIENT_ID, igdb_id);
  SetDlgItemTextString(dlg, IDC_SETTINGS_IGDB_CLIENT_SECRET, igdb_secret);
  if (credentials_changed) {
    settings.igdbAccessToken.clear();
    settings.igdbTokenExpiresUtc.clear();
    for (auto& entry : state->games) {
      entry.igdbMetadataFetched = false;
    }
    state->igdb_status = L"IGDB credentials updated. Fetch a new token to refresh metadata.";
    UpdateStatusBar(state, state->igdb_status);
  }
  UpdateIgdbStatusLabel(dlg, settings);
...
INT_PTR CALLBACK SettingsDialogProc(HWND dlg, UINT msg, WPARAM wparam, LPARAM lparam) {
  ...
  switch (msg) {
    case WM_INITDIALOG: {
      ...
      SetDlgItemTextString(dlg, IDC_SETTINGS_IGDB_CLIENT_SECRET, settings.igdbClientSecret);
      UpdateIgdbStatusLabel(dlg, settings);
    ...
    case WM_COMMAND: {
      switch (LOWORD(wparam)) {
        ...
        case IDC_SETTINGS_IGDB_FETCH_TOKEN: {
          std::wstring client_id = Trim(GetDlgItemTextString(dlg, IDC_SETTINGS_IGDB_CLIENT_ID));
          std::wstring client_secret = Trim(GetDlgItemTextString(dlg, IDC_SETTINGS_IGDB_CLIENT_SECRET));
          if (client_id.empty() || client_secret.empty()) {
            MessageBoxW(dlg, L"Enter both the IGDB Client ID and Secret before fetching a token.", L"IGDB",
                        MB_ICONWARNING);
            return TRUE;
          }
          if (!context || !context->app_state) {
          ...
          std::wstring token = context->app_state->settings.igdbAccessToken;
          std::wstring expires = context->app_state->settings.igdbTokenExpiresUtc;
          std::wstring error;
          if (IGDB::EnsureAccessToken(client_id, client_secret, token, expires, error)) {
            context->app_state->settings.igdbClientId = client_id;
            context->app_state->settings.igdbClientSecret = client_secret;
            context->app_state->settings.igdbAccessToken = token;
            context->app_state->settings.igdbTokenExpiresUtc = expires;
            for (auto& entry : context->app_state->games) {
              entry.igdbMetadataFetched = false;
            }
            UpdateIgdbStatusLabel(dlg, context->app_state->settings);
            SettingsManager::Save(context->app_state->settings);
            context->app_state->igdb_status = L"IGDB token fetched successfully.";
            UpdateStatusBar(context->app_state, context->app_state->igdb_status);
            MessageBoxW(dlg, L"IGDB access token fetched successfully.", L"IGDB", MB_ICONINFORMATION);
          } else {
            std::wstring message = L"Failed to fetch IGDB token.";
            if (!error.empty()) {
              message.append(L"\n");
              message.append(error);
            }
            MessageBoxW(dlg, message.c_str(), L"IGDB", MB_ICONERROR);
            if (!error.empty()) {
              context->app_state->igdb_status = error;
              UpdateStatusBar(context->app_state, context->app_state->igdb_status);
            }
            UpdateIgdbStatusLabel(dlg, context->app_state->settings);
          }
...
void PerformScan(HWND hwnd, AppState* state) {
  ...
  if (state->settings.igdbClientId.empty() || state->settings.igdbClientSecret.empty()) {
    state->igdb_status =
        L"IGDB integration not configured. Provide credentials in Settings to download covers and metadata.";
    UpdateStatusBar(state, state->igdb_status);
  } else if (state->igdb_status.find(L"IGDB") == std::wstring::npos) {
...
void OnPaint(HWND hwnd, AppState* state) {
  ...
  if (!state->games.empty() && state->selected_index < state->games.size()) {
    ...
    if (game.igdbHasRating || !game.igdbSummary.empty() || !game.igdbGenres.empty() || !game.igdbPlatforms.empty()) {
      details_y += 12;
      state->renderer->DrawText(L"IGDB", details_x, details_y, highlight_color);
      details_y += 18;

      if (game.igdbHasRating) {
        std::wostringstream rating_stream;
        rating_stream << std::fixed << std::setprecision(1) << game.igdbRating;
        std::wstring rating_line = L"Rating: " + rating_stream.str();
        DrawMultiline(state->renderer.get(), rating_line, details_x, details_y, meta_color);
        details_y += 18;
      }

      if (!game.igdbSummary.empty()) {
        std::wstring summary_text = L"Summary: " + game.igdbSummary;
        DrawMultiline(state->renderer.get(), summary_text, details_x, details_y, text_color, 18);
        std::vector<std::wstring> summary_lines = SplitLines(summary_text);
        details_y += static_cast<int>(summary_lines.size() * 18);
      }

      if (!game.igdbGenres.empty()) {
        std::wstring genres_line = L"Genres: " + JoinWithSeparator(game.igdbGenres, L", ");
        DrawMultiline(state->renderer.get(), genres_line, details_x, details_y, meta_color);
        details_y += 18;
      }

      if (!game.igdbPlatforms.empty()) {
        std::wstring platforms_line = L"Platforms: " + JoinWithSeparator(game.igdbPlatforms, L", ");
        DrawMultiline(state->renderer.get(), platforms_line, details_x, details_y, meta_color);
        details_y += 18;
      }
    }
