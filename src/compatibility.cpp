...
#endif
  WinHttpSetOption(session.get(), WINHTTP_OPTION_SECURE_PROTOCOLS, &secure_protocols, sizeof(secure_protocols));
...
std::vector<IndexEntry> ParseIndex(const std::wstring& text) {
  ...
  for (const auto& line : SplitLines(text)) {
    ...
    size_t mdOpen = trimmed.find(L'[');
    if (mdOpen != std::wstring::npos) {
      size_t mdClose = trimmed.find(L']', mdOpen);
      if (mdClose != std::wstring::npos && mdClose > mdOpen) {
        size_t parenOpen = trimmed.find(L'(', mdClose);
        if (parenOpen != std::wstring::npos && parenOpen == mdClose + 1) {
          size_t parenClose = trimmed.find(L')', parenOpen);
          if (parenClose != std::wstring::npos && parenClose > parenOpen + 1) {
            std::wstring title = trimmed.substr(mdOpen + 1, mdClose - mdOpen - 1);
            std::wstring target = trimmed.substr(parenOpen + 1, parenClose - parenOpen - 1);
            if (!title.empty() && !target.empty() && ToLower(title) != L"template") {
              entries.push_back({title, target});
            }
            continue;
          }
        }
...
std::wstring ReadWikiText(const std::filesystem::path& path) {
  std::wstring text;
  if (!Cache::ReadText(path.wstring(), text)) {
    return L"";
  }
  return text;
}
...
std::vector<std::filesystem::path> DiscoverWikiRoots() {
  std::vector<std::filesystem::path> roots;
#ifdef _WIN32
  if (wchar_t* env = _wgetenv(L"OPTISCALER_WIKI_DIR")) {
    if (*env) {
      std::filesystem::path envPath(env);
      if (std::filesystem::is_directory(envPath)) {
        roots.push_back(envPath);
      }
    }
  }
#endif
  ...
  for (int depth = 0; !current.empty() && depth < 5; ++depth) {
    std::filesystem::path candidate = current / L"OptiScaler.wiki";
    if (std::filesystem::is_directory(candidate)) {
      roots.push_back(candidate);
    }
    std::filesystem::path thirdParty = current / L"third_party" / L"OptiScaler.wiki";
    if (std::filesystem::is_directory(thirdParty)) {
      roots.push_back(thirdParty);
    }
    current = current.parent_path();
  }
  std::wstring appData = Cache::AppDataRoot();
  if (!appData.empty()) {
    std::filesystem::path appWiki = std::filesystem::path(appData) / L"wiki" / L"OptiScaler.wiki";
    if (std::filesystem::is_directory(appWiki)) {
      roots.push_back(appWiki);
    }
  }
  std::vector<std::filesystem::path> unique;
  for (const auto& root : roots) {
    if (std::find(unique.begin(), unique.end(), root) == unique.end()) {
...
bool RefreshCompatibilityFromLocal(const std::wstring& wikiRoot, CompatibilityCache& cache,
                                   std::vector<std::wstring>& logLines, std::wstring& statusMessage) {
  namespace fs = std::filesystem;
  cache.profiles.clear();
  if (wikiRoot.empty()) {
    statusMessage = L"Local compatibility wiki path not configured.";
    return false;
  }

  fs::path root(wikiRoot);
  if (!fs::is_directory(root)) {
    statusMessage = L"Local compatibility wiki not found at " + wikiRoot + L".";
    return false;
  }

  std::error_code canonicalError;
  fs::path canonical = fs::weakly_canonical(root, canonicalError);
  if (!canonicalError && fs::is_directory(canonical)) {
    root = canonical;
  }

  const fs::path indexCandidates[] = {root / L"Compatibility-List.asciidoc", root / L"Compatibility-List.adoc",
                                      root / L"Compatibility-List.md"};
  std::wstring indexBody;
  fs::path indexPath;
  for (const auto& candidate : indexCandidates) {
    if (fs::exists(candidate)) {
      indexBody = ReadWikiText(candidate);
      if (!indexBody.empty()) {
        indexPath = candidate;
        break;
      }
  ...
  if (indexBody.empty()) {
    statusMessage = L"Local compatibility index not found under " + wikiRoot + L".";
    return false;
  }

  auto entries = ParseIndex(indexBody);
  if (entries.empty()) {
    statusMessage = L"No entries discovered in local compatibility index.";
    return false;
  }

  if (!indexPath.empty()) {
    logLines.push_back(L"Using local compatibility index " + indexPath.wstring());
  }

  std::vector<CompatibilityProfile> profiles;
  profiles.reserve(entries.size());
  for (const auto& entry : entries) {
    std::vector<fs::path> pageCandidates;
    if (!entry.slug.empty()) {
      pageCandidates.push_back(root / (entry.slug + L".asciidoc"));
      pageCandidates.push_back(root / (entry.slug + L".adoc"));
      pageCandidates.push_back(root / (entry.slug + L".md"));
    }
    if (pageCandidates.empty()) {
      std::wstring fallbackSlug = SlugFromTitle(entry.title);
      if (!fallbackSlug.empty()) {
        pageCandidates.push_back(root / (fallbackSlug + L".asciidoc"));
        pageCandidates.push_back(root / (fallbackSlug + L".adoc"));
        pageCandidates.push_back(root / (fallbackSlug + L".md"));
      }
    }

    std::wstring pageBody;
    for (const auto& candidate : pageCandidates) {
      if (fs::exists(candidate)) {
        pageBody = ReadWikiText(candidate);
        if (!pageBody.empty()) {
          break;
        }
      }
    ...
    if (pageBody.empty()) {
      std::wstring slugLabel = entry.slug.empty() ? L"" : L" (" + entry.slug + L")";
      logLines.push_back(L"Missing compatibility page for " + entry.title + slugLabel);
      continue;
    }

    std::vector<std::wstring> warnings;
    CompatibilityProfile profile = ParseProfileFromBody(entry.title, pageBody, warnings);
    std::wstring slugHint = NormalizeToken(entry.slug);
    if (!slugHint.empty() &&
        std::find(profile.exeHints.begin(), profile.exeHints.end(), slugHint) == profile.exeHints.end()) {
      profile.exeHints.push_back(slugHint);
    }
    if (!warnings.empty()) {
      profile.parseWarnings = warnings;
      for (const auto& warn : warnings) {
  ...
  if (profiles.empty()) {
    statusMessage = L"No compatibility profiles parsed from local wiki.";
    return false;
  }

  logLines.push_back(L"Loaded " + std::to_wstring(profiles.size()) + L" profiles from local wiki.");
  cache.profiles = std::move(profiles);
  cache.meta.fetchedUtc = Iso8601NowUtc();
  cache.meta.sourceUrl = L"local:" + root.wstring();
  cache.meta.etag.clear();
  cache.meta.lastModified.clear();
  cache.meta.count = cache.profiles.size();
  statusMessage = L"Loaded " + std::to_wstring(cache.meta.count) + L" compatibility profiles from local wiki.";
  return true;
}

std::wstring FindBundledCompatibilityWiki() {
  namespace fs = std::filesystem;
  auto roots = DiscoverWikiRoots();
  for (const auto& root : roots) {
    std::error_code ec;
