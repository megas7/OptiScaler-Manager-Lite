...
std::wstring SupportSummaryForSource(const std::wstring& source) {
  const std::wstring lower = ToLower(source);
  if (lower == L"steam") {
    return L"Steam integration: overlay, cloud saves, achievements";
  }
  if (lower == L"epic") {
    return L"Epic Games Launcher features supported";
  }
  if (lower == L"xbox") {
    return L"Microsoft Store / Xbox services where available";
  }
  return L"Manual install – configure features in OptiScaler";
}
...
std::vector<GameEntry> Scanner::ScanAll(const std::vector<std::wstring>& roots) {
  ...
  for (const auto& root : roots) {
    ...
    while (it != end) {
      ...
      game.source = source;
      game.officialSupport = SupportSummaryForSource(game.source);
