...
bool ShouldSkipExecutable(const std::wstring& filename_lower) {
  constexpr const wchar_t* kPrefixes[] = {
      L"unins",         L"uninstall",      L"setup",        L"dxsetup",    L"vc_redist",  L"vcredist",
      L"helper",        L"crashreport",    L"readme",       L"support",    L"patch",      L"update",
      L"redistributable", L"cleanup",      L"repair",       L"diagnostic", L"config",     L"utility"};
  ...
  constexpr const wchar_t* kSubstrings[] = {
      ...
      L"launcher",       L"autoupdate",    L"benchmark",       L"server",      L"crashpad_handler",
      L"reporting",      L"errorreport",   L"diagnostics",     L"configurator", L"installer",
      L"updater",        L"patcher",       L"tool",            L"editor",      L"helper",
      L"support",        L"telemetry"};
...
bool ShouldSkipPath(const std::filesystem::path& path) {
  ...
  constexpr const wchar_t* kIgnored[] = {
      ...
      L"\\thirdparty\\",    L"\\redistlist\\",  L"\\utility\\",      L"\\installer\\"};
