...
bool SettingsManager::Load(SettingsData& settings) {
  ...
  try {
    ...
    settings.igdbClientId = WideFromUtf8(doc.value("igdbClientId", std::string()));
    settings.igdbClientSecret = WideFromUtf8(doc.value("igdbClientSecret", std::string()));
    settings.igdbAccessToken = WideFromUtf8(doc.value("igdbAccessToken", std::string()));
    settings.igdbTokenExpiresUtc = WideFromUtf8(doc.value("igdbTokenExpiresUtc", std::string()));
    EnsureDefaults(settings);
...
bool SettingsManager::Save(const SettingsData& settings) {
  ...
  doc["igdbClientId"] = Utf8FromWide(tmp.igdbClientId);
  doc["igdbClientSecret"] = Utf8FromWide(tmp.igdbClientSecret);
  doc["igdbAccessToken"] = Utf8FromWide(tmp.igdbAccessToken);
  doc["igdbTokenExpiresUtc"] = Utf8FromWide(tmp.igdbTokenExpiresUtc);
